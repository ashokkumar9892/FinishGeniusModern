...
export function PhotoReader({ groupId, onRead, storedFile, onPhoto }: {
  ...
  const [active, setActive] = useState<'wood' | 'card'>('wood')
  ...
  const read = useMutation({
    mutationFn: async () =>
      (await api.post<PhotoReading>('/color-matching/read-photo', {
        groupId, storedFile: file, sample: wood, card, cardType,
      })).data,
    onSuccess: (d) => {
      setReading(d)
      onRead(d)
  ...
  return (
    <div className="space-y-3">
      <Field label="Photograph" hint="A fixed camera, even lighting and a grey card in the shot give a reading worth keeping.">
      ...
      {file && (
        <>
            ...
            <div className="inline-flex rounded-md border bg-muted/40 p-0.5" role="group" aria-label="What to mark next">
              {(['wood', 'card'] as const).map((k) => (
                <button
                  ...
                  onClick={() => setActive(k)}
                >
                  {k === 'wood' ? 'Mark the wood' : 'Mark the card'}
                </button>
              ))}
            </div>
            {active === 'card' && (
          ...
          <RegionPicker
            ...
            active={active}
            onChange={(k, r) => (k === 'wood' ? setWood(r) : setCard(r))}
            regions={[
